...
const staffService = {
  ...
  async recentRentals(staff, { page = 1, limit = 20, status, search } = {}) {
    // Staff must still have an assigned station to use the return
    // workflow (enforced here and, authoritatively, in returnService),
    // but which station they're assigned to must NOT restrict which
    // rentals they can see: pickup station != return station. This
    // listing is intentionally campus-wide.
    requireStationAssignment(staff);
    const skip = (page - 1) * limit;
    const where = {};
